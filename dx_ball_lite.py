"""
This game is contributed by Ashok Arora as part of team 'hava'
...
def start_dx_ball_lite():  # main function that starts the game
    root = Tk()
    root.title("DX Ball Lite")
            ...
            while True:
                if paddle.pausec != 1:
                    ...
                    if not ball.bottom_hit:
                        ...
                        root.update()
                        time.sleep(0.01)
                        if ball.hit == 95:  # a lot of balls have been hit
                            canvas.create_text(
                                250, 250, text="YOU WON !!", fill="yellow", font="Consolas 24 ")
                            root.update_idletasks()
                            root.update()
                            playing = False
                            break
                    else:  # ball goes beyond the paddle
                        canvas.create_text(
                    ...
                    try:
                        ...
                        m = canvas.create_text(
                            250, 250, text="PAUSE!!", fill="green", font="Consolas 24 ")
                    root.update_idletasks()
                    root.update()
    ...
    canvas.create_text(
        250, 250, text="Press Enter to start Game!!", fill="white", font="Consolas 18")
    j = canvas.find_all()
    root.mainloop()
